...
def D(p, z, version='simplified'): # negative cosine similarity
    ...
    elif version == 'simplified':# same thing, much faster. Scroll down, speed train_0.1 in __main__
...
class SimSiam(nn.Module):
    def __init__(self, backbone=resnet50(), out_dim=512):
        super().__init__()

        self.backbone = backbone
        self.projector = projection_MLP(out_dim)
...
class Satellite(nn.Module):
    def __init__(self, backbone=resnet50(), out_dim=0):
        super().__init__()
        self.backbone = backbone
        # self.output_dim = backbone.output_dim

    def forward(self, x1, x2, enable):
        f = self.backbone
        y1, y2 = f(x1, enable), f(x2, enable)

        # random_loss = torch.randn(1, device=x1.device)  # 在当前设备上生成一个随机标量损失
        # return {'loss': random_loss}
