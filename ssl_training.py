...
def main(device, args):
    ...
    # 冻结模型
    # if args.freeze:
    #     for p in model.backbone.backbone.conv1.parameters():
    #         p.requires_grad = False
    #     for p in model.backbone.backbone.layer1.parameters():
    #         p.requires_grad = False
    #     for p in model.backbone.backbone.layer2.parameters():
    #         p.requires_grad = False
    #     for p in model.backbone.backbone.layer3.parameters():
    #         p.requires_grad = False
    ...
    for epoch in global_progress:
        ...
        for idx, ((images1, images2), labels) in enumerate(local_progress):
            ...
            # if ((idx+1) % accum_iter == 0) or idx + 1 == len(local_progress):
            #     # 平均累积的梯度
            #     if idx > int(len(local_progress)/8) * 8:
            #         model.zero_grad()
            #         break
            ...
            #             param.grad /= accum_iter  # 对梯度进行平均
                        ...
                        #     print("梯度爆炸！检测到 NaN 或 Inf")
                        #     break
                        # if idx + 1 == len(local_progress):
                        #     param.grad /= 29   # len(local_progress)%accum_iter = 29
            # start_time = time.time()
            optimizer.step()
            model.zero_grad()
            lr_scheduler.step()
    ...
    # Save checkpoint
    model_path = os.path.join(args.ckpt_dir, f"pre_acc{accuracy:.1f}_all_time{all_time:.2f}mins"+args.dataset.name + '_lr' + str(args.train.base_lr) +
